...
layout=[]
route_Listbox=[]
chat_metrobot_layout=["[sg.Multiline('test text',False,True,s=(30,10),text_color='green',background_color='cyan',auto_size_text=10)]","[sg.Button('Talk To Metrobot')]"]
font=("Segoe Print", 10)
...
def input(x):
    ...
    tabgrp=[[sg.TabGroup([[sg.Tab('Tab1',list(map(eval,layout))),sg.Tab('Chat n Ask with Metrobot',list(map(eval,chat_metrobot_layout)))]]),sg.Column([[sg.Image('Img.png')],[sg.Button('Close')]])]]
    window=sg.Window('Window title',tabgrp,font=font).Finalize()
    #window.Maximize()
